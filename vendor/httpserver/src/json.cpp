...
const Value& Value::operator[] (std::size_t i) const
{
 ...
 void Value::clear(){
    ...
    std::list<std::pair<std::string,json::Value>>::const_iterator Object::begin() const
    {
        return _object.begin();
    }

    std::list<std::pair<std::string,json::Value>>::const_iterator Object::end() const
    {
        return _object.end();
    }

    std::list<std::pair<std::string,json::Value>>::iterator Object::begin()
    {
        return _object.begin();
    }

    std::list<std::pair<std::string,json::Value>>::iterator Object::end()
    {
        return _object.end();
    }
    ...
    std::string  encode(json::Value& v){
        ...
        switch(v.type())
        {
            ...
            case STRING:
                os <<'"'<< JSON_UTF8_TO_ASCII((std::string)v)<<'"';                
                break;
            ...
            if(jsonstr[j]==0x5c)//'\'
            {
                ...
                switch(jsonstr[j+1]){
                    ...
                    auto chartoint=[](unsigned char& ch,unsigned char& c){ 
                        if(ch>='0' && ch<='9') c=(unsigned char)(ch-'0');
                        if(ch>='a' && ch<='f') c=(unsigned char)(ch-'a'+10);
                        if(ch>='A' && ch<='F') c=(unsigned char)(ch-'A'+10); 
                    };
    ...
    int JSON_OBJ(std::string &jsonstr,Object &obj,int offset){
        int i=0,nobj=0;
    ...
    int JSON_ARRAY(std::string &jsonstr,Array &obj,int offset){
        int i=0;
        std::string key,value;
    ...
    Value decode(std::string &&jsonstr){
        Object obj;    
        int offset=0;
    ...
    Value decode(std::string &&jsonstr,bool isarray){
        Array obj;
        int offset=0;
        int zlsize=jsonstr.length();
        if(zlsize>5000000){
    ...
    bool save_json(std::string &&path,std::string &content)
    {
...
{    
    switch(v.type())
    {
        ...
        case json::STRING:
            os <<'"'<< json::JSON_UTF8_TO_ASCII((std::string)v)<<'"';                
            break;
