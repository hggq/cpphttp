...
int viewcli(){
    ...
    std::string viewcache="cache/view/";
    ...
    tp.DisplayDirTree("cache/view",0,false);
    std::map<std::string,time_t>  mustfile=tp.contrasttime("cache/");
    int i=1;
    ...
    if(mustfile.size()>0){
        ...
        while(i>0){
            ...
            if(commond=="r"||commond=="R"){
                 tp.DisplayDirTree("view",0,true);
                 tp.DisplayDirTree("cache/view",0,false);
                 mustfile=tp.contrasttime("cache/");
                 i=1;
                  for(auto it=mustfile.begin();it!=mustfile.end();it++){
                        std::cout <<i<<" \033[32m[+]\033[0m \033[1m\033[31m" << it->first << "\033[0m ";
                        tm* lsystemTime = localtime(&it->second);
            ...
            if(commond.size()>0){
                if(commond[0]=='a'||commond[0]=='A'){
                        ...
                        for(auto it=mustfile.begin();it!=mustfile.end();it++){
                                ...
                                std::cout <<k<<" \033[32m[+]\033[0m \033[1m\033[31m" << it->first << "\033[0m ->"<<tmpath.string()<<" ->\033[32m"<<tp.sofilename<<"\033[0m "<<std::endl;
